...
public class AccountManager {
		...
		try {
			...
			if (account_number != 0) {
				PreparedStatement preparedStatement = connection
						.prepareStatement("SELECT * FROM AccountsBanky WHERE account_number = ? and security_pin = ? ");
				preparedStatement.setLong(1, account_number);
				preparedStatement.setString(2, security_pin);
				ResultSet resultSet = preparedStatement.executeQuery();

				if (resultSet.next()) {
					String credit_query = "UPDATE AccountsBanky SET balance = balance + ? WHERE account_number = ?";
			...
			if (account_number != 0) {
				...
				if (resultSet.next()) {
					...
					if (amount <= current_balance) {
						String debit_query = "UPDATE AccountsBanky SET balance = balance - ? WHERE account_number = ?";
	...
	public void getBalance(long account_number) {
		...
		try {
			PreparedStatement preparedStatement = connection
					.prepareStatement("SELECT balance FROM AccountsBanky WHERE account_number = ? AND security_pin = ?");
			preparedStatement.setLong(1, account_number);
			preparedStatement.setString(2, security_pin);
			ResultSet resultSet = preparedStatement.executeQuery();
			if (resultSet.next()) {
